...
func edit_function(function: FunctionLogicResource):
	...
	for child in $VBoxContainer/ScrollContainer/Code.get_children():
		child.queue_free()
	
	$VBoxContainer/ScrollContainer/Code.add_child(InstructionVisualBuilder.instantiate(function))
	$VBoxContainer/FunctionName.text = function.name
...
func createArgUI(startingIdx:int, count : int):
	for i in range(count):
		...
		$VBoxContainer/ArgumentsEdition/Arguments.add_child(argUI)


func _on_delete_pressed() -> void:
	var argIdx = $VBoxContainer/ArgumentsEdition/Arguments.get_child_count() - 1
	$VBoxContainer/ArgumentsEdition/Arguments.get_child(argIdx).queue_free()
